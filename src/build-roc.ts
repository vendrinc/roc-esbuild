...
const buildRocFile = (
  ...
  // Generate the C glue and the .d.ts file
  runRoc(["glue", path.join(__dirname, "node-glue.roc"), rocBuildOutputDir, rocPlatformMain])

  // Move the .roc.d.ts file to be in the same dir as the .roc file, because that's what tooling wants.
  const dtsFileName = rocFileName + ".d.ts"
  fs.renameSync(path.join(rocBuildOutputDir, dtsFileName), path.join(rocFileDir, dtsFileName))
  ...
  const cGluePath = path.join(rocBuildOutputDir, "node-to-roc.c")
